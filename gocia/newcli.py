...
@click.option("--output", "-o",
              default="gocia_progress.html",
              show_default=True,
              type=click.Path(dir_okay=False),
              help="Output HTML file.")
def cmd_plot(run_dir: str, output: str) -> None:
    """
    Write an interactive HTML progress dashboard for the current run.

    Reads gocia.db and produces a self-contained HTML file with four panels:

    \b
      1. GCE vs generation  — best-so-far step line, mean ± 1σ band,
                               individual structure scatter coloured by operator
      2. Status breakdown   — stacked bar of converged / desorbed / failed
                               / isomer counts per generation
      3. Mean coverage      — mean adsorbate count per species in the
                               selectable pool, per generation
      4. Operator rate      — fraction of offspring per operator that
                               reached converged or isomer
    ...
    \b
        gocia plot
        gocia plot --output week1.html
        gocia plot --run-dir /scratch/myrun --output /tmp/dash.html
    """
    run_dir_path = Path(run_dir)
    db_path = run_dir_path / "gocia.db"
    ...
    try:
        ...
        click.echo(
            "plotly is required for `gocia plot`. "
    ...
    try:
        ...
        click.echo(
            "pandas is required for `gocia plot`. "
    ...
    if df.empty:
        click.echo("Database is empty — nothing to plot.")
        raise SystemExit(0)

    output_path = Path(output)
    run_label = run_dir_path.resolve().name

    build_dashboard(df, output_path=output_path, run_label=run_label)
